test_that("simple linear regression by gradient descent is (approximately) correct", {
  ...
  my_result <- slr_gd(mtcars, "mpg", "hp")

  mass_result <- lm(mpg ~ hp, data = mtcars)

  expect_equal(coef(mass_result)[['hp']], my_result$final_slope,
               tolerance = 0.05, scale = abs(my_result$final_slope))
  expect_equal(coef(mass_result)[[1]], my_result$final_intercept,
               tolerance = 0.05, scale = abs(my_result$final_intercept))
...
test_that("multiple linear regression by gradient descent is correct", {
  ...
  expect_equal(coef(mass_result)[['hp']], my_result$final_coefficients[1],
               tolerance = 0.05, scale = abs(my_result$final_coefficients[1]))
  expect_equal(coef(mass_result)[['cyl']], my_result$final_coefficients[2],
               tolerance = 0.05, scale = abs(my_result$final_coefficients[2]))
  expect_equal(coef(mass_result)[[1]], my_result$final_intercept,
               tolerance = 0.05, scale = abs(my_result$final_intercept))
